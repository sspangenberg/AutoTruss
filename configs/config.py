...
def get_base_config():
    ...
    parser.add_argument(
        "--coordinate_range",
        type=list,
        default=[(0.0, 4.634), (-0.483, 0.7725), (-0.5, 1.0)],
        help="points' range",
    )
    parser.add_argument(
        "--area_range", type=list, default=(0.0001, 0.003028), help="edges' area range"
    )
    parser.add_argument(
        "--len_range", type=list, default=(0.03, 5.0), help="edges' length range"
    )
    parser.add_argument(
        "--area_delta_range",
        type=list,
        default=(-0.0005, 0.0005),
        help="edges' area delta range",
    )
    parser.add_argument(
        "--coordinate_delta_range",
        type=list,
        default=[(-0.5715, 0.5715), (-0.5715, 0.5715), (-0.5715, 0.5715)],
        help="nodes' coordinate delta range",
    )
    ...
    parser.add_argument(
        "--input-path", type=str, default="input/kr-sundial-newinput.txt"
    )
    parser.add_argument("--ratio_ring", default=0.0)
    parser.add_argument(
        "--fixed_points", type=int, default=4, help="number of fixed nodes"
    )
    parser.add_argument(
        "--variable_edges",
        type=int,
        default=-1,
        help="number of variable edges, -1 if all is variable",
    )
    ...
    parser.add_argument(
        "--transfer-filefold", type=str, default="DIVERSITY_TOPO_result"
    )
    ...
    parser.add_argument(
        "--num_trains_per_train_loop", type=int, default=5, help="for sac training"
    )
    parser.add_argument(
        "--num_train_loops_per_epoch", type=int, default=5, help="for sac training"
    )
    parser.add_argument(
        "--hidden-dims", type=list, default=[256, 512], help="hidden layer dimensions"
    )
    ...
    parser.add_argument(
        ...
        type=list,
        default=[128, 256],
        help="input dims for TransformerEmbed",
    )
    parser.add_argument(
        "--post-dims",
        type=list,
        default=[256, 128],
        help="hidden dims for TransformerEmbed",
    )
    parser.add_argument(
        "--max-refine-steps",
        type=int,
        default=20,
        help="maximum timesteps of an episode",
    )
    parser.add_argument(
        "--EmbeddingBackbone",
        type=str,
        default="Transformer",
        help="Transformer or GNN",
    )
