...
class GNNEMBED(_Mlp):
    ...
    def forward(self, *inputs, **kwargs):
        ...
        edge_id, edge_feature = Edge_IndexEmbedding(
        ...
        for i in range(flat_inputs.shape[0]):
            graph_data = Data(
                x=embed_node_outputs[i],
                edge_index=edge_id[i].transpose(0, 1),
                edge_attr=edge_feature[i],
            )
            graph_out = self.gcn1(
                x=graph_data.x,
            ...
            if id_inputs[i, 1] != -1:
                ...
                embed_graph_out = self.edge_query(
                    torch.cat((graph_out[u], graph_out[v]))
                )
            if id_inputs[i, 2] != -1:
        ...
        feature = self.mix_graph_action(
            torch.cat((embed_graph_outs, embed_act_outputs), dim=-1)
        )
...
class GNNEMBED_policy(TanhGaussianPolicy):
    ...
    def __init__(
        self, input_dims, action_dim, hidden_dims, env_dims, env_mode, num_point
    ):
        super().__init__(
    ...
    for k in range(_pos_inputs.shape[0]):
        ...
        while idx < num_points * (num_points - 1) / 2:
            ...
            if env_mode == "DT" and edge_inputs[k][idx * 2] > 0:
                one_output.append([i, j])
                one_feature_output.append(
                    [edge_inputs[k][idx * 2], edge_inputs[k][idx * 2 + 1]]
                )
            idx += 1
            j += 1
            if j == num_points:
